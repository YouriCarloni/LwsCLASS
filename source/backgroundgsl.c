...
/* Numerical integration for CLP equation of state using GSL */
#include <stdio.h>
#include <math.h>
#include <gsl/gsl_integration.h>
#include "background.h"

/* Function to integrate: 3 * (1 + w_fld(a)) with log(a) as variable */
double integrand_fld_gsl(double log_a, void *params) {
    struct background *pba = (struct background *) params;

    double a = exp(log_a); // log(a) -> a

    double ww0 =
        -1.0
        + (((1.0 + pba->w0_fld)) / (pow(a, -3)))
          * pow(
              atanh(
                  pow(
                      (pow(a, -3) * (-1.0 + 1.0 / (pba->Omega0_fld)))
                      / (1.0 + (-1.0 + 1.0 / (pba->Omega0_fld)) * pow(a, -3)),
                      0.5
                  )
              )
              - atanh(pow(2.0, -0.5)),
          ...
          * pow(
              atanh(pow(1.0 - pba->Omega0_fld, 0.5))
              - atanh(pow(2.0, -0.5)),
              -2.0
          );
...
/* --- start: integration for QQ fluid --- */

if (pba->fluid_equation_of_state == QQ) {

  /* Integration limits in log(a) */
  double log_a_ini = log(1e-14);
  double log_a_0   = log(1.0);  /* = 0.0 */

  gsl_integration_workspace *w = gsl_integration_workspace_alloc(1000);

  gsl_function F;
  F.function = &integrand_fld_gsl;
  F.params   = (void *) pba;
  ...
  /* gestione errore in stile CLASS (se vuoi essere elegante) */
