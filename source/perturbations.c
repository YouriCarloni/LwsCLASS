    ...
    for (index_k=0; index_k<ppt->k_size[index_md]; index_k++) {
      ...
      if (output_format == class_format) {
        ...
        if (ppt->has_density_transfers == _TRUE_) {
          ...
          class_store_double(dataptr,tk[ppt->index_tp_delta_new],ppt->has_source_delta_new,storeidx);
        ...
        if (ppt->has_velocity_transfers == _TRUE_) {
          ...
          class_store_double(dataptr,tk[ppt->index_tp_theta_new],ppt->has_source_theta_new,storeidx);
        ...
        class_store_double_or_default(dataptr,-tk[ppt->index_tp_delta_new]/k2,ppt->has_source_delta_new,storeidx,0.0);
...
int perturbations_output_titles(
  ...
  if (output_format == class_format) {
    ...
    if (ppt->has_density_transfers == _TRUE_) {
      ...
      class_store_columntitle(titles,"d_new",pba->has_new);
    ...
    if (ppt->has_velocity_transfers == _TRUE_) {
      ...
      class_store_columntitle(titles,"t_new",pba->has_new);
...
int perturbations_indices(
  ...
  ppt->has_source_delta_new = _FALSE_;
  ...
  ppt->has_source_theta_new = _FALSE_;
    ...
    if (_scalars_) {
      ...
      if (ppt->has_density_transfers == _TRUE_) {
        ...
        if (pba->has_new == _TRUE_)
          ppt->has_source_delta_new = _TRUE_;
      ...
      if (ppt->has_velocity_transfers == _TRUE_) {
        ...
        if (pba->has_new == _TRUE_)
          ppt->has_source_theta_new = _TRUE_;
        if (pba->has_scf == _TRUE_)
      ...
      class_define_index(ppt->index_tp_delta_new,  ppt->has_source_delta_new, index_type,1);
      ...
      class_define_index(ppt->index_tp_theta_new,  ppt->has_source_theta_new, index_type,1);
...
int perturbations_prepare_k_output(struct background * pba,
  ...
  if (ppt->k_output_values_num > 0) {
    ...
    if (ppt->has_scalars == _TRUE_){
      ...
      /** New matter fluid */
      class_store_columntitle(ppt->scalar_titles, "delta_new", pba->has_new);
      class_store_columntitle(ppt->scalar_titles, "theta_new", pba->has_new);
...
int perturbations_vector_init(
  ...
  if (_scalars_) {
    ...
    class_define_index(ppv->index_pt_delta_new,pba->has_new,index_pt,1); /* new matter fluid density */
    class_define_index(ppv->index_pt_theta_new,pba->has_new,index_pt,1); /* new matter fluid velocity */
  ...
  /** - case of switching approximation while a wavenumber is being integrated */
    ...
    if (_scalars_) {
      ...
      if (pba->has_new == _TRUE_) {
        ...
        ppv->y[ppv->index_pt_delta_new] =
          ppw->pv->y[ppw->pv->index_pt_delta_new];

        ppv->y[ppv->index_pt_theta_new] =
          ppw->pv->y[ppw->pv->index_pt_theta_new];
      }
    ...
    */
    ...
    if ((ppt->has_ad == _TRUE_) && (index_ic == ppt->index_ic_ad)) {
      ...
      if (pba->has_idm == _TRUE_) {
       ...
       /* new matter fluid */
      if (pba->has_new == _TRUE_) {
        ppw->pv->y[ppw->pv->index_pt_delta_new] = (3.* (1. + pba->w_new))/4.*ppw->pv->y[ppw->pv->index_pt_delta_g]; /* new fluid density */
        ppw->pv->y[ppw->pv->index_pt_theta_new] = ppw->pv->y[ppw->pv->index_pt_theta_g]; /* new fluid velocity */
      }
    ...
    if (ppt->gauge == newtonian) {
      ...
      */
      ...
      if (pba->has_new == _TRUE_) {
        ppw->pv->y[ppw->pv->index_pt_delta_new] -= 3.* (1. + pba->w_new)*a_prime_over_a*alpha;
        ppw->pv->y[ppw->pv->index_pt_theta_new] += k*k*alpha;
      }
...
int perturbations_total_stress_energy(
  ...
  if (_scalars_) {
    ...
    */
    ...
    /* new matter fluid contribution */
    if (pba->has_new == _TRUE_) {
      ppw->delta_rho += ppw->pvecback[pba->index_bg_rho_new]*y[ppw->pv->index_pt_delta_new];
      ppw->rho_plus_p_theta += (1. +pba->w_new) * ppw->pvecback[pba->index_bg_rho_new]*y[ppw->pv->index_pt_theta_new];
      ppw->delta_p += pba->w_new * ppw->pvecback[pba->index_bg_rho_new]*y[ppw->pv->index_pt_delta_new];
      ppw->rho_plus_p_tot += (1 + pba->w_new) * ppw->pvecback[pba->index_bg_rho_new];
    }
...
int perturbations_sources(
  ...
  if (_scalars_) {
    ...
    /* delta_new */
    if (ppt->has_source_delta_new == _TRUE_) {
      _set_source_(ppt->index_tp_delta_new) =  y[ppw->pv->index_pt_delta_new]
        + 3.*a_prime_over_a*(1.+pba->w_new)*theta_over_k2; // N-body gauge correction
    }
    ...
    /* theta_new */
    if (ppt->has_source_theta_new == _TRUE_) {
     _set_source_(ppt->index_tp_theta_new) = y[ppw->pv->index_pt_theta_new]
        + theta_shift; // N-body gauge correction
    }
...
int perturbations_print_variables(double tau,
  ...
  double delta_new=0.,theta_new=0.;
  ...
  if (_scalars_) {
    ...
    /* New matter fluid */
    class_store_double(dataptr, delta_new, pba->has_new, storeidx);
    class_store_double(dataptr, theta_new, pba->has_new, storeidx);
    /** Fluid */
...
int perturbations_derivs(double tau,
  ...
  if (_scalars_) {
    ...
    if (pba->has_new == _TRUE_) {
        ...
        dy[pv->index_pt_delta_new] =
          -(1+pba->w_new)*(y[pv->index_pt_theta_new]+metric_continuity);

        /** - ----> fluid velocity */
        dy[pv->index_pt_theta_new] = /* fluid velocity */
          -(1.-3.*pba->w_new)*a_prime_over_a*y[pv->index_pt_theta_new]
          +pba->w_new*k2/(1.+pba->w_new)*y[pv->index_pt_delta_new]
          +metric_euler;
    }
