...
int background_functions(
  ...
  /* new type of matter*/ 
  if (pba->has_new == _TRUE_) {
    pvecback[pba->index_bg_rho_new] = pba->Omega0_new * pow(pba->H0,2)/pow(a, 3.0 * (1.0 + pba->w_new));
    rho_tot += pvecback[pba->index_bg_rho_new];
    p_tot += (pba->w_new) * pvecback[pba->index_bg_rho_new];
    dp_dloga += -pba->w_new * 3.0 * (1.0 + pba->w_new) * pvecback[pba->index_bg_rho_new];
  }
  ...
  if (rho_r == 0 || rho_crit == 0) {
    printf("Warning: rho_r or rho_crit is zero. Forcing Omega_r to a valid value.\n");
    rho_r = 5.0e-5 * pow(pba->H0, 2);
    rho_crit = pow(pba->H0, 2);
  }
...
// Define the equation of state parameter w_fld(a)
#include <stdio.h>
#include <stdlib.h>
#include <math.h>
#include "background.h"
...
int background_w_fld(
  ...
  case OS1:
    *w_fld = pba->w0_fld + pba->wa_fld * (1. - a);
    break;
  case OS2:
    *w_fld = -1. - ((-1. + a)*a*pba->w0_fld*sin((-1.+pow(a, -1.)*pba->wa_fld)))/(1.+2.*(-1.+a)*a);
    break;
  case QQ:
  ...
  case PADE:
    *w_fld = (pba->w0_fld)/(1.+ pba->wb_fld* (1./a - 1.));
    break;
  case PADE1:
    *w_fld = (pba->w0_fld + pba->wc_fld * (1./a - 1.))/(1. + pba->wb_fld * (1./a - 1.));
    break;
  case PADE2:
  ...
  case OS1:
    *dw_over_da_fld =- pba->wa_fld;
    break;
  case OS2:
    *dw_over_da_fld = ((-1. + a)*(1. + 2. * (-1. +a )*a) * pba->w0_fld * pba->wa_fld * cos((-1.+pow(a, -1.)*pba->wa_fld))+(1. - 2*a)*a*pba->w0_fld*sin((-1.+pow(a, -1.))*pba->wa_fld))/(a*pow((1.+2.*(-1+a)*a),2.));
    break;
  case QQ:
  ...
  case PADE:
    *dw_over_da_fld = (pba->w0_fld * pba->wb_fld)/(pow(pba->wb_fld + a - pba->wb_fld * a, 2.0));
    break;
  case PADE1:
    *dw_over_da_fld = (pba->w0_fld * pba->wb_fld - pba->wc_fld)/(pow(pba->wb_fld + a - pba->wb_fld * a, 2.0));
    break;
  case PADE2:
  ...
  case PADE:
    *integral_fld = (3.0 * pba->w0_fld * log((1.0 - a) * pba->wb_fld + a) + (3.0 - 3.0 * pba->wb_fld) * log(a))/(pba->wb_fld - 1.0);
    break;
  case PADE1:
    *integral_fld = (3.0 * (- ((pba->wb_fld + pba->wc_fld) * log(a)) + ((pba->w0_fld * pba->wb_fld - pba->wc_fld) * log(a + pba->wb_fld - a* pba->wb_fld))/(-1.0 + pba->wb_fld)))/pba->wb_fld;
    break;
  case PADE2:
    *integral_fld = 0.0;
        ;
...
int background_init(
  ...
  /* ====> Add this to compute the modified sound horizon */          
  class_call(background_rs_modified(pba),
           pba->error_message,
           pba->error_message);
           
    /* ====> Add this to compute the modified sound horizon */          
  class_call(background_rd_modified(pba),
           pba->error_message,
           pba->error_message);
...
int background_indices(
  ...
  class_define_index(pba->index_bg_rho_dr,pba->has_dr,index_bg,1);
    ...
    /* - index for new fluid */
  class_define_index(pba->index_bg_rho_new,pba->has_new,index_bg,1);
...
int background_initial_conditions(
  ...
  */
  ...
  /** DEBUG PRINT: Check initial Omega_r 
  printf("DEBUG: Initial Omega_r = %e\n", pvecback[pba->index_bg_Omega_r]);

  /** FIX: Ensure Omega_r is not zero or too small 
  if (pvecback[pba->index_bg_Omega_r] < 1e-6) {
      printf("Warning: Omega_r too small, forcing it to a valid value.\n");
      pvecback[pba->index_bg_Omega_r] = 1e-3;  // Set a small but valid radiation fraction
  }**/
...
/* Romberg Integration for CLP equation of state */
#include <stdio.h>
#include <math.h>
#include "background.h"

#define ROMBERG_MAX 10  // Maximum refinement steps
#define EPSILON 1e-8     // Convergence criterion

/* Function to integrate: 3 * (1 + w_fld(a)) with improved transformation */
double integrand_fld(double log_a, struct background *pba) {
    double a = exp(log_a); // Transform log(a) back to a
    double ww0 =  -1 + (((1+pba->w0_fld))/(pow(a, -3)))*pow(atanh(pow((pow(a,-3)*(-1+1/(pba->Omega0_fld)))/(1+(-1+1/(pba->Omega0_fld))*pow(a,-3)), 0.5))-atanh(pow(2, -0.5)), 2)*pow( atanh(pow(1-pba->Omega0_fld,0.5))-atanh(pow(2, -0.5)),-2);

    return (3 * (1 + ww0)); // Removed division by 'a'
}

/* Romberg Integration Method with Logarithmic Scale Factor */
double romberg(double (*f)(double, struct background *), double log_a1, double log_b, size_t max_steps, double acc, struct background *pba) {
    double *R1 = (double*)malloc(max_steps * sizeof(double));
    double *R2 = (double*)malloc(max_steps * sizeof(double));
    if (R1 == NULL || R2 == NULL) {
        printf("Memory allocation failed\n");
        return NAN;
    }
    double *Rp = R1, *Rc = R2; // Rp is previous row, Rc is current row
    double h = log_b - log_a1; // step size
    Rp[0] = (f(log_a1, pba) + f(log_b, pba)) * h * 0.5; // first trapezoidal step
    ...
    double result = Rp[max_steps-1]; // return our best guess
    free(R1);
    free(R2);
    return result;
}

if (pba->fluid_equation_of_state == QQ) {
    double log_a_ini = log(1e-14);
    double log_a_0 = log(1.0);
    integral_fld = romberg(integrand_fld, log_a_ini, log_a_0, 10, 1e-6, pba);
} else {
...
pvecback_integration[pba->index_bi_rho_fld] = rho_fld_today * exp(integral_fld);
  ...
  class_test(fabs(pvecback[pba->index_bg_Omega_r]-1.) > ppr->tol_initial_Omega_r,
             pba->error_message,
    /*class_test(fabs(pvecback[pba->index_bg_Omega_r]-1.) > 1e-3,
             pba->error_message,// Increase tolerance*/
...
int background_output_titles(
  ...
  class_store_columntitle(titles,"(.)rho_new",pba->has_new);
  ...
  class_store_columntitle(titles,"rs_mod",_TRUE_);
  class_store_columntitle(titles,"rd_mod",_TRUE_);
...
int background_output_data(
  ...
  for (index_tau=0; index_tau<pba->bt_size; index_tau++) {
    ...
    class_store_double(dataptr,pvecback[pba->index_bg_rho_new],pba->has_new,storeidx);
    ...
    class_store_double(dataptr,pba->rs_mod,_TRUE_,storeidx);
    class_store_double(dataptr,pba->rd_mod,_TRUE_,storeidx);
...
int background_output_budget(
  ...
  if (pba->background_verbose > 1) {
    ...
    if ((pba->has_fld == _TRUE_)|| (pba->has_lambda == _TRUE_)  || (pba->has_scf == _TRUE_) || (pba->has_curvature == _TRUE_)) {
...
int background_rs_modified(struct background *pba) {
  ...
  int N_steps = 100000; // must be even!
  if (N_steps % 2 != 0) N_steps++;

  double dlogz = (logz_start - logz_end) / N_steps;
  double z, logz, H, R, S, cs, integrand, rs = 0.0;
  int last_index = 0;
  double *pvecback;
  ...
  for (int i = 0; i <= N_steps; i++) {
    ...
    H = pvecback[pba->index_bg_H];
    R = 3.0 * pba->Omega0_b / (4.0 * pba->Omega0_g);
    S = 3.0 * (1.0 + pba->w_new) * pba->Omega0_new / (4.0 * pba->Omega0_g);

    double z_factor = pow(1.0 + z, 3.0 * pba->w_new - 1.0);
    cs = sqrt((1.0 + 3.0 * pba->w_new * S * z_factor) /
              (3.0 * (1.0 + R / (1.0 + z) + S * z_factor)));
  ...
  /*printf(">>> Final optimized rs_mod = %.6f Mpc\n", rs);*/

  return _SUCCESS_;
}

int background_rd_modified(struct background *pba) {
  double logz_start = log(1e6);
  double logz_end = log(1060);
  int N_steps = 100000; // must be even!
  if (N_steps % 2 != 0) N_steps++;

  double dlogz = (logz_start - logz_end) / N_steps;
  double z, logz, H, R, S, cs, integrand, rd = 0.0;
  int last_index = 0;
  double *pvecback;
 ...
 /* printf(">>> Final optimized rd_mod = %.6f Mpc\n", rd);*/
